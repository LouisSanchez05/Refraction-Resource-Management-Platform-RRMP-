...
function MembershipManagementPage() {
  ...
  async function handleCreatePlan(event) {
    ...
    if (
      ...
      setError('Monthly hours must be zero or greater.');
      return;
    }
    ...
    if (
      ...
      setError('Overage rate must be zero or greater.');
      return;
    }
    ...
    try {
      ...
      setPlanMessage('Membership plan created successfully.');
    } catch (err) {
      setError(err.message);
    } finally {
  ...
  async function handleUpdatePlan(planIdToUpdate) {
    const name = editPlanName.trim();
    const monthlyHours = Number(editMonthlyHours);
    const overageRate = Number(editOverageRate);
    ...
    try {
      const updatedPlan = await apiRequest(
        `/api/memberships/plans/${planIdToUpdate}`,
        {
          method: 'PATCH',
          body: JSON.stringify({
            name,
  ...
  return (
    <main>
      ...
      <p>
        Assign monthly membership plans to companies and manage
